<template>
  ...
  >
    <v-card-title style="color: deepskyblue">Member Report</v-card-title>
    <v-card
      color="transparent"
      flat
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        { title: "Year", key: "year" },
        { title: "Births", key: "birth" },
        { title: "Deaths", key: "death" },
  ...
  methods: {
    async getReport() {
      try {
        ...
        const data = await fetchApi(`/api/report/member`, {
          method: "POST",
          body: JSON.stringify({
        ...
        console.error("Get Member Report thất bại:", err);
